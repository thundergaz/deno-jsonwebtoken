...
import { isString, isNumber } from "./assertType.ts";
...
export function ms(val: string, options?: { long?: boolean }): number | undefined;
export function ms(val: number, options?: { long?: boolean }): string;
export function ms(val: string|number,options?: { long?: boolean }): string|number|undefined {
  if (isString(val) && val.length > 0) return parse(val);
  else if (isNumber(val) && !isNaN(val)) return options?.long ? fmtLong(val) : fmtShort(val);
